/*
    Original Code:
    https://github.com/gupon/Houdini-Gulib/blob/master/vex/include/bezier_util_2d.vfl
*/
...
void intersectionLineBezier(vector2 curve[]; vector2 line[]; float resultA[]; float resultB[]) {
    ...
    */
    function int pointInLineSegment(vector2 p; vector2 l0; vector2 l1)
    {
        ...
        if (l0.x == l1.x) return p.y >= ymin && p.y <= ymax;
        if (l0.y == l1.y) return p.x >= xmin && p.x <= xmax;
